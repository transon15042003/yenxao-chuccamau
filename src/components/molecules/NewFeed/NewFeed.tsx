...
export const NewFeed = ({ initialBlogs, loading, className }: NewFeedProps) => {
  const [blogsToDisplay, setBlogsToDisplay] = useState<BlogPost[]>(initialBlogs || []);

  useEffect(() => {
    if (initialBlogs) {
      setBlogsToDisplay(initialBlogs);
    }
  }, [initialBlogs]);

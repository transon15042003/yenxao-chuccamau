...
import { getRegionHybrid } from './regions-hybrid';
 ...
 */
export async function retrieveCartHybrid(cartId?: string) {
  const id = cartId;
...
export async function createCartHybrid(countryCode: string) {
  const region = await getRegionHybrid(countryCode).catch(console.error);

  if (!region) {
    throw new Error(`Region not found for country code: ${countryCode}`);
  }

  let cart = await retrieveCartHybrid();
